...
probe11 = cppimport.imp("addictif.common.fenicstools.cpp.probe11")
#probe11 = cppimport.imp("cpp.probe11")
...
# Give the compiled classes some additional pythonic functionality
class GradProbe(probe11.GradProbe):
...
class GradProbes(probe11.GradProbes):
    ...
    def array_grad(self, N=None, filename=None, component=None, root=0):
        """Dump data to numpy format on root processor for all or one snapshot."""
        is_root = comm.Get_rank() == root
        size = self.get_total_number_probes() if is_root else len(self)
        comp = self.value_size() * self.geom_dim() if component is None else 1
        if not N is None:
            z  = zeros((size, comp))
        else:
            z  = zeros((size, comp, self.number_of_evaluations()))
        ...
        if len(self) > 0:
            if not N is None:
                for k in range(comp):
                    if is_root:
                        ids = self.get_probe_ids()
                        z[ids, k] = self.get_probes_grad_component_and_snapshot(k, N)
                    else:
                        z[:, k] = self.get_probes_grad_component_and_snapshot(k, N)
            else:
                for i, (index, probe) in enumerate(self):
                    j = index if is_root else i
                    if not N is None:
                        z[j, :] = probe.get_probe_grad_at_snapshot(N)
                    else:
                        for k in range(self.value_size()):
                            z[j, k, :] = probe.get_probe_grad_sub(k)
